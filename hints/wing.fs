...
let xWingFind (candidateLookup : Cell -> Set<Candidate>) (houseCells : House -> Set<Cell>) (rs : Row list) (cs : Column list) = 
...
let yWingsPerHouseCandidate (candidateLookup : Cell -> Set<Candidate>) (houseCells : House -> Set<Cell>) (house1 : House) (house2 : House) houseCandidateCells1 houseCandidateCells2 (candidate : Candidate) = 
    let hht1 = Set.filter (fun (candidates, _) -> Set.contains candidate candidates) houseCandidateCells1
    let hht2 = Set.filter (fun (candidates, _) -> Set.contains candidate candidates) houseCandidateCells2
...
let yWingsPerHouse (candidateLookup : Cell -> Set<Candidate>) (houseCells : House -> Set<Cell>) (row1 : Row) (row2 : Row) (col1 : Column) (col2 : Column) = 
                    ...
                    | left :: pivot :: right :: _ ->
                        ...
                        if Set.count removee = 1 && ((fst left) <> (fst right)) && 
                            ...
                            Some { HintDescription.candidateReductions = set [ candidateReductions ]
                                   primaryHouses = set [ Row row1; Row row2; Column col1; Column col2 ]
                                   secondaryHouses = set []
                                   pointers = pointers
                                   setCellValue = None }
                        else None
...
let yWingFind (candidateLookup : Cell -> Set<Candidate>) (houseCells : House -> Set<Cell>) (rows : Row list) (cols : Column list) = 
    ...
    let hints =
        Seq.mapi (fun i row1 ->
            Seq.mapi (fun j row2 ->
                Seq.mapi (fun k col1 ->
                    Seq.mapi (fun l col2 ->
                        yWingsPerHouse candidateLookup houseCells row1 row2 col1 col2) (Seq.skip (k + 1) cols)) cols) (Seq.skip (i + 1) rows)) rows
