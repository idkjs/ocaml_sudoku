...
let columnStack (boxWidth : int<width>) (column : Column) = 
    match column with
    | { col = col } -> 1 + ((int) col - 1) / (int) boxWidth
                       |> makeStack
...
let rowBand (boxHeight : int<height>) (row : Row) = 
    match row with
    | { row = row } -> 1 + ((int) row - 1) / (int) boxHeight
                       |> makeBand
...
let stacks (boxWidth : int<width>) (length : int) = List.map makeStack [ 1..(length / (int) boxWidth) ]

let bands (boxHeight : int<height>) (length : int) = List.map makeBand [ 1..(length / (int) boxHeight) ]
...
let columns length = List.map makeColumn [ 1..length ]

let rows length = List.map makeRow [ 1..length ]
...
let contentsOfContainer<'a, 'b when 'b : equality> (s : 'b) (sc : Container<'b, 'a>) = 
    List.filter (fun p -> s = fst p) sc |> List.map (fun p -> snd p)

let containerOfContent<'a, 'b when 'a : equality> (sc : Container<'b, 'a>) (c : 'a) = 
    List.find (fun p -> c = snd p) sc |> fst

let makeContainerContainerCell<'a when 'a : equality> (columns : 'a list) (allCells : Cell list) 
    (container : Cell -> 'a) = 
...
let makeContainerBoxCell (boxes : Box list) (cells : Cell list) boxWidth boxHeight = 
    let (boxCells, boxCellLookup) = 
        makeContainerContainerCell<Box> boxes cells (fun cell -> 
            let col = cell.col
